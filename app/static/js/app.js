...
let manualDisconnect = false; // Flag to prevent auto-reconnect when manually switching modes
...
function connectWebsocket() {
  ...
  websocket.onclose = function () {
    ...
    if (!manualDisconnect) {
      ...
      manualDisconnect = false; // Reset the flag
    }
  };
...
startAudioButton.addEventListener("click", () => {
  ...
  // Close existing connection and reconnect with audio mode
  if (websocket && websocket.readyState === WebSocket.OPEN) {
    manualDisconnect = true; // Prevent auto-reconnect
    websocket.close();
    // Wait a bit before reconnecting to ensure clean disconnection
    setTimeout(() => {
      connectWebsocket();
    }, 100);
  } else {
...
stopAudioButton.addEventListener("click", () => {
  ...
  // Keep audio styling class and audio mode active
  // Don't close the WebSocket - just stop recording
  // This allows the agent to process the audio and respond
});
